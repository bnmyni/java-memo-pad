...
@Transactional
public class ReportControllerTest extends AbstractContextControllerTests {
...
//		request.setSub_pid(460L);
		...
		this.mockMvc.perform(post("/report/progress")
	...
	public void top5Test() throws Exception {
		...
		request.setYears(2018);
//		request.setPid(90);
		request.setLimits(30);
		ObjectMapper mapper = new ObjectMapper();
		ObjectWriter ow = mapper.writer().withDefaultPrettyPrinter();
		String json = ow.writeValueAsString(request);

		this.mockMvc.perform(post("/report/top5")
				.contentType(MediaType.APPLICATION_JSON).content(json))
				.andExpect(content().contentType("application/json;charset=utf-8")).andReturn();
	}
}
