...
public class ManyTdOverDegCalHandler implements OverDegreeCalHandle{
	...
	private static final int overDag = ConfigurationManager.getDefaultConfig().getInteger("TDMANYOVERDAG_OVERDAG", 80);
	//多补一TD单个邻区采样点占总采样点比例(0-100,默认30)
	private static final int singleNc_rate = ConfigurationManager.getDefaultConfig().getInteger("TDMANYOVERDAG_SINGLE_NC_RATE", 30);
	...
	public void finalOperation(OverlayDegreeSetting setting, PrintStream ps){
		for(Entry<String, ManyOverDegCalModel> entry:manyMap.entrySet()) {
			...
			if(manyInfoModel.getCount() > 0){
				...
				int trueCellCount = overlayDegreeDao.queryTrueTdCellCoint(manyInfoModel.getQuerySql());
				
				List<Map<String, Object>> cellDataList = manyInfoModel.getCellDataList();
				//所有可补偿邻区采样点占总采样点比例>80%的场合
