...
@Component("tdHwKpiCalHandle")
public class TdHwKpiCalHandle implements KpiCalHandle {
    ...
    public String handle(KpiCalModel model) {
        ...
        List<Map<String, Object>> dataList = kpiCalDao.quertDataBySql(model
        ...
        try {
            ...
            for (Map<String, Object> data : dataList) {
                double yyyw = FormatUtil.tranferCalValue(data
                        .get("rlc_cstraffic_conv"));// 语音业务
                double sjll = (FormatUtil.tranferCalValue(data
                        .get("rlc_psuloct")) + FormatUtil.tranferCalValue(data
                        .get("rlc_psdloct"))) / 1024;// 数据流量
                double mzylyl = 0;//
                mzylyl = (FormatUtil.tranferCalValue(data
                        .get("cr_meannbrassnbrudl")) + FormatUtil
                        .tranferCalValue(data.get("cr_meannbrassnbruul")))
                        / ((80 + 32) * FormatUtil.tranferCalValue(data.get("zpgs")) + (64 + 32)
                        * FormatUtil.tranferCalValue(data.get("fpgs")))
                        / 0.75;
                double zdyhs = FormatUtil.tranferCalValue(data
                        .get("hsdpa_maxnbruser"));
                double dyhsl = sjll * 1024 / zdyhs / 15 / 60;// 单用户速率=业务流量/HSDPA最大用户数/15/60
                ...
                double psyll = FormatUtil.tranferCalValue(data
                        .get("rlc_psuloct"))
                        + FormatUtil.tranferCalValue(data.get("rlc_psdloct"));
