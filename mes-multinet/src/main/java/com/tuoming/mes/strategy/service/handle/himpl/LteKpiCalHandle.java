	...
	public String handle(KpiCalModel model) {
		...
		try {
			...
			for (Map<String, Object> data : dataList) {
				double sxsjll = FormatUtil.tranferCalValue(data
						.get("pdcp_upoctul")) / 1024;// ：上行数据流量
				double xxsjll = FormatUtil.tranferCalValue(data
						.get("pdcp_upoctdl")) / 1024;// 下行数据流量
				double zdyhs = FormatUtil.tranferCalValue(data
						.get("rrc_connmax"));// 最大用户数
				double dyhsl = (FormatUtil.tranferCalValue(data
						.get("pdcp_upoctul")) + FormatUtil.tranferCalValue(data
						.get("pdcp_upoctdl")))
						/ FormatUtil.tranferCalValue(data.get("rrc_connmax"));// 单用户速率
				double prblyl = (FormatUtil.tranferCalValue(data
						.get("rru_puschprbtotmeanul")) + FormatUtil
						.tranferCalValue(data.get("rru_pdschprbtotmeandl")))
						/ (FormatUtil.tranferCalValue(data
								.get("rru_puschprbmeantot")) + FormatUtil
								.tranferCalValue(data
										.get("rru_pdschprbmeantot")));// PRB利用率
