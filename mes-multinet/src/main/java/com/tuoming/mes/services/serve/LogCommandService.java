...
public interface LogCommandService extends BaseService<LogCommand, String> {
     ...
     *
     * @param groupName
     *         要执行采集的分组名称，对应aos_log_command表的group_name字段
     * @param batch
     *         采集批次标识
     */
     ...
     *
     * @param objectType
     *         要采集的对象类型标识，大部分情况下我们都是使用BSC（注意，这里不区分2/3g我们统一都叫BSC）
     * @param manufacturers
     *         要执行采集的厂家标识，对应aos_manufacturers表配置
     * @param batch
     *         采集批次标识
     */
     ...
     *
     * @param queryCommand
     *         要执行日志采集的查询指令名，对应aos_log_command表中采集指令名称的配置
     * @param batch
     *         采集批次标识
     */
     ...
     *         要执行采集的服务器列表
     * @param queryCommand
     *         要执行的采集指令名称，对应aos_log_command中的配置
     * @param batch
     *         采集操作批次号
     */
     ...
     *
     * @param server
     *         要执行采集的服务器名称,对应aos_servers表内配置
     * @param queryCommand
     *         要执行的采集指令名称，对应aos_log_command指令配置
     * @param batch
     *         采集操作批次号
     */
     ...
     *
     * @param logFile
     *         要解析的日志文件路径，这里可以使用相对路径或绝对路径
     * @param logParser
     *         对应的解析器名称，即aos_log_parser中的配置名称
     */
