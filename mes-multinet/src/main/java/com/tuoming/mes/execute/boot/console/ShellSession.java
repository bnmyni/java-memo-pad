...
import com.pyrlong.dsl.integration.impl.MapVariableResolverFactory;
import com.pyrlong.dsl.templates.TemplateRuntime;
import com.pyrlong.dsl.util.StringAppender;
...
public class ShellSession {
    ...
    public ShellSession(Map<String, String> env) {
        ...
        env.put(PROMPT_VAR, Constants.PROMPT);
        ...
        env.put("$COMMAND_PASSTRU", "true");
        env.put("$PRINTOUTPUT", "true");
        env.put("$ECHO", ConfigurationManager.getDefaultConfig().getBoolean("pyrlong.hamster.shell.echo", false).toString());
        env.put("$SHOW_TRACES", "true");
        env.put("$USE_OPTIMIZER_ALWAYS", "true");
        env.put("$PATH", "");

        try {
            ResourceBundle bundle = getBundle("dsl.properties");
    ...
    private void _exec() {
        ...
        if ("true".equals(env.get("$ECHO")) && StringUtil.isNotEmpty(commandBuffer)) {
        ...
        if (inTokens.length != 0 && commands.containsKey(inTokens[0])) {
            ...
            try {
                ...
                String[] args = inBuffer.toString().split(" ");
                if (args.length == 0)
                    return;
                if (args.length > 1) {
                    Application.evalFile(Application.getProjectName(), args[0], args[1]);
                } else {
                    Application.evalFile(args[0]);
                }
                outputBuffer = "";
            } catch (Exception e) {
            ...
            if (outputBuffer != null && "true".equals(env.get("$PRINTOUTPUT"))) {
                if (outputBuffer.getClass().isArray()) {
                    ...
                    out.println(StringUtil.toString(outputBuffer));
                }
            }
    ...
    public void run(ParserContext context) {
        pCtx = context;// DSLUtil.getDefaultInstance();
        run();
    }
    ...
    public void run() {
        ...
        try {
            while (true) {
                ...
                cmdStarted = DateUtil.getTimeinteger();
                cmdIsRunning = true;
                _exec();
                cmdStarted = DateUtil.getTimeinteger();
                cmdIsRunning = false;
