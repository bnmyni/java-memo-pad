...
(function($) {
						...
						function(e) {
							...
							if (e.pageX < p2 && e.pageX > p1) {
								...
								if (pos >= 0) {
									...
									//ȡsortName by haomingli
									if(col.sortName){
										_78 = col.sortName;
									}
									//addbyzhuangruhai ���ϼ�¼�������
									var sortOrder = $(this).parent().attr("_sortOrder");
									if (sortOrder){
										_7b = sortOrder == 'asc' ? 'desc': 'asc';
									}
									$(this).parent().attr("_sortOrder",_7b);
									//end
									if (_74.multiSort) {
										_79.push(_78);
										_7a.push(_7b);
	...
	function _8d(_8e) {
		...
		function _99(col, _9a) {
			col.width += _9a;
			col.boxWidth += _9a;
			_90.find("td[field=\"" + col.field + "\"] div.datagrid-cell")
					.width(col.boxWidth);
		}
		;
		function _94(col) {
	...
	function _9b(_9c, _9d) {
		...
		function _19(_a1) {
			var _a2 = dc.view.find("div.datagrid-header td[field=\"" + _a1
					+ "\"] div.datagrid-cell");
			_a2.css("width", "");
			...
			var _a3 = Math.max(_a2._outerWidth(), _a4("allbody"),
					_a4("allfooter"));
			_a2._outerWidth(_a3);
			col.width = _a3;
			...
			function _a4(_a5) {
				var _a6 = 0;
				_9e.finder.getTr(_9c, 0, _a5).find(
						"td[field=\"" + _a1 + "\"] div.datagrid-cell").each(
						function() {
	...
	function _51(_a7, _a8) {
		...
		function fix(_ad) {
			var col = _6f(_a7, _ad);
			if (!col.checkbox) {
				_a9.ss.set("." + col.cellClass, col.boxWidth ? col.boxWidth
						+ "px" : "auto");
			}
		}
		;
			...
			if (_c6.idField) {
				for ( var i = 0; i < _c4.rows.length; i++) {
					...
					if (_cd(_c5.selectedRows, row)) {
						_c6.finder.getTr(_c3, i).addClass(
								"datagrid-row-selected");
					}
					if (_cd(_c5.checkedRows, row)) {
						_c6.finder.getTr(_c3, i).find(
		...
		if (tr.length) {
			...
			if (top < 0) {
				...
				if (top + tr._outerHeight() > _e5.height() - 18) {
					_e5.scrollTop(_e5.scrollTop() + top + tr._outerHeight()
							- _e5.height() + 18);
				}
			}
		}
	...
	function _e7(_e8, _e9) {
		...
		var _eb = _ea.options;
		_eb.finder.getTr(_e8, _ea.highlightIndex).removeClass(
				"datagrid-row-over");
	...
	function _f3(_104, _105) {
		...
		opts.finder.getTr(_104, "", "selected").removeClass(
				"datagrid-row-selected");
		if (opts.idField) {
	...
	function _f4(_10a, _10b, _10c) {
		...
		var ck = opts.finder.getTr(_10a, _10b).find(
				"div.datagrid-cell-check input[type=checkbox]");
	...
	function _fc(_10f, _110, _111) {
		...
		var ck = opts.finder.getTr(_10f, _110).find(
				"div.datagrid-cell-check input[type=checkbox]");
	...
	function _102(_114, _115) {
		...
		var bck = opts.finder.getTr(_114, "", "allbody").find(
				"div.datagrid-cell-check input[type=checkbox]");
	...
	function _108(_117, _118) {
		...
		var dc = _119.dc;
		var hck = dc.header1.add(dc.header2).find("input[type=checkbox]");
		var bck = opts.finder.getTr(_117, "", "allbody").find(
				"div.datagrid-cell-check input[type=checkbox]");
	...
	function _16d(_16e, _16f) {
		...
		var tr = opts.finder.getTr(_16e, (_16f.index != undefined ? _16f.index
				: _16f.id));
		if (!tr.length) {
			return;
		}
	...
	var _175 = {
		...
		combobox : {
			...
			setValue : function(_1ad, _1ae) {
				...
				if (opts.multiple) {
					if (_1ae) {
						$(_1ad).combobox("setValues",
								_1ae.split(opts.separator));
					} else {
						$(_1ad).combobox("clear");
					}
				} else {
	...
	$.fn.datagrid.methods = {
						...
						if (opts.loadMsg) {
							...
							if (!_1c3.children("div.datagrid-mask").length) {
								...
								var msg = $(
										...
										.html(opts.loadMsg).appendTo(_1c3);
		...
		updateRow : function(jq, _1d9) {
			return jq
					.each(function() {
						var opts = $.data(this, "datagrid").options;
						opts.view.updateRow.call(opts.view, this, _1d9.index,
								_1d9.row);
					});
		},
	...
	$.fn.datagrid.parseOptions = function(_1e2) {
		var t = $(_1e2);
		return $.extend({}, $.fn.panel.parseOptions(_1e2), $.parser
				.parseOptions(_1e2, [ "url", "toolbar", "idField", "sortName",
	...
	$.fn.datagrid.parseData = function(_1e3) {
		...
		var _1e4 = t.datagrid("getColumnFields", true).concat(
	...
	var _1e5 = {
		render : function(_1e6, _1e7, _1e8) {
			...
			for ( var i = 0; i < rows.length; i++) {
				...
				_1eb.push("<tr id=\"" + _1ef + "\" datagrid-row-index=\"" + i
						+ "\" " + cls + " " + _1ee + ">");
				_1eb.push(this.renderRow.call(this, _1e6, _1ea, _1e8, i,
						rows[i]));
				_1eb.push("</tr>");
			}
		...
		renderFooter : function(_1f0, _1f1, _1f2) {
			...
			for ( var i = 0; i < rows.length; i++) {
				_1f4.push("<tr class=\"datagrid-row\" datagrid-row-index=\""
						+ i + "\">");
				_1f4.push(this.renderRow.call(this, _1f0, _1f3, _1f2, i,
						rows[i]));
				_1f4.push("</tr>");
			}
		...
		renderRow : function(_1f5, _1f6, _1f7, _1f8, _1f9) {
			...
			for ( var i = 0; i < _1f6.length; i++) {
				...
				if (col) {
					...
					var _1ff = col.hidden ? "style=\"display:none;" + _1fe
							+ "\"" : (_1fe ? "style=\"" + _1fe + "\"" : "");
					...
					cc.push(">");
					if (col.checkbox) {
						cc.push("<input type=\"checkbox\" name=\"" + _1fb
								+ "\" value=\""
								+ (_1fc != undefined ? _1fc : "") + "\">");
					} else {
						if (col.formatter) {
							
							var formatter_=$("<div>"+col.formatter(_1fc, _1f9, _1f8)+"<div>");
							var a_=formatter_.find("a");
							if(a_ && a_.is("a") && dataLimit){
								var text=a_.text();
								var html=a_.html();
								var _text=dataLimit(text,col.width);
								if (text==html &&_text.indexOf("<div>")>-1 && $(_text).is("div")){
									a_.text($(_text).text());
									a_.attr("title",text);
									cc.push(formatter_.html());
								}else{
							...
							//add by lipeng  用于表格字符串截取
							if(dataLimit){
								_1fc=dataLimit(_1fc,col.width);
							}
							cc.push(_1fc);
						}
					}
		...
		updateRow : function(_202, _203, row) {
			...
			var css = opts.rowStyler ? opts.rowStyler.call(_202, _203,
					rows[_203]) : "";
			var _204 = "";
			var _205 = "";
			if (typeof css == "string") {
			...
			var _204 = "datagrid-row "
					+ (_203 % 2 && opts.striped ? "datagrid-row-alt " : " ")
					+ _204;
			function _206(_207) {
				...
				var _209 = tr.find(
						...
						":checked");
				tr.html(this.renderRow.call(this, _202, _208, _207, _203,
						rows[_203]));
		...
		onAfterRender : function(_21e) {
			...
			if (opts.showFooter) {
				var _21f = $(_21e).datagrid("getPanel").find(
						"div.datagrid-footer");
				_21f
						.find(
					...
					$.fn.panel.defaults,
					{
						...
						finder : {
							getTr : function(_225, _226, type, _227) {
								...
								if (_227 == 0) {
									var tr1 = opts.finder.getTr(_225, _226,
											type, 1);
									var tr2 = opts.finder.getTr(_225, _226,
											type, 2);
									return tr1.add(tr2);
								} else {
									if (type == "body") {
										var tr = $("#" + _228.rowIdPrefix + "-"
												+ _227 + "-" + _226);
										if (!tr.length) {
											tr = (_227 == 1 ? dc.body1
													: dc.body2)
										...
										if (type == "footer") {
											return (_227 == 1 ? dc.footer1
													: dc.footer2)
													.find(">table>tbody>tr[datagrid-row-index="
															+ _226 + "]");
										} else {
											if (type == "selected") {
												return (_227 == 1 ? dc.body1
														: dc.body2)
