jQuery(function($) {
	...
	jqGrid_init($(grid_selector), pager_selector, {
		url : window.ctxPaths + "/servapp/servapp/myApply.ajax",
		shrinkToFit: true,
		forceFit: true,
		sortable : true,
		sortname : 'createTime',
		...
		onSelectRow:function(rowid,status){
            $("#apply-table tr").each(function(i){
	...
	function addActions(cellvalue, options, rowObject) {
		var url = 'pages/serv/Businessreception_list.shtml?from_page=myApply&biz_id='+rowObject.bizId;
		var op = '查看';
		if (rowObject.bizStatus == 'res') {
			op = '修改';								
			url = 'pages/serv/Businessreception_res.shtml?from_page=myApply&biz_id='+rowObject.bizId;
		}
		return '<div>'
				+ "<a target='mainFrame' href='javascript:;' onclick=signLastSelect(this) url='"+url+"'>"+op+"</a>"
				+ '</div>';
	}
});

function signLastSelect(_this){	
	$('#gview_apply-table').find('tr').css('background','');
	$(_this).parents('tr').css('background','#E4EFC9');
	
	 var hoverName ="ui-state-highlight",
	 $trObj =$("#apply-table tr"),
	 _this;
     $trObj.each(function(){
         _this = $(this).attr("class");
         if(_this.indexOf(hoverName) !== -1){
	...
	var node = $(tempstr);
		...
		if(this.endStatus=='Y'){
			...
			if(typeof(this.endOpinion) == "undefined" 
				...
				node.find('.div_endLeader').show();
				node.find(".endLeaderName").text(this.dendLeaderName);
...
$('.js-print-all').on('click',function() {
	...
	function processData(newtables) {
		...
		$overlay.find('form:gt(0)').each(function() {
...
$('#apply-table').on('click','a', function(){
...
$('#btn_search').on('click', function() {
	jQuery('#apply-table').jqGrid('setGridParam', {
