	...
	<script type="text/javascript">
		...
		function bindData(dialog, tabId) {
			...
			for(var i=0;i<dialogQueryArr.length;i++){
					...
					$(":text#"+dialogQueryObj.condition,".fieldTr",currentDiv).each(function(){
		...
		function getCurrentTableType(){
			...
			if(name.length < 2){
				if(obj.parents("div[type='subtable']").length >0){
					isMainTable = false;
					currentTableName = obj.parents("div[type='subtable']").attr("tablename").toLowerCase();
				}
				else {
					isMainTable = true;
				}
