...
//检测office控件是否安装和是否加载成功
function officeInit(){
	TANGER_OCX_OBJ = document.getElementById("TANGER_OCX");
	try {  
		var ax = new ActiveXObject("NTKO.OfficeControlQiYe");  
	} catch(e) {  
		alert("office 控件未安装，或被禁止了"); 
		document.write('<SPAN STYLE="color:red">不能装载文档控件。请在检查浏览器的选项中检查浏览器的安全设置。请点击<a href="'+__ctx +'/media/office/NtkoOfficeControlSetup.msi">安装控件</a></SPAN>');
		return 0;
	}  
	try{
		alert(TANGER_OCX_OBJ.GetOfficeVer());
	}
	catch(e)
	{
		alert("office控件加载出现出错,"+e.message);
	}
