$(function(){
	...
	$("input[name='m:me_lyhtztbg:hth']").change(function(){
		var hth = $(this).val();
		if(!hth) return;
		if(hth.length != 9){
...
function me_lyhtztbg_wyftkAddRowAfterEvent(curRow){
	var preRow = $(".listRow:visible",$("[tablename='me_lyhtztbg_wyftk']")).last();
...
$().ready(function (){
	...
	$("div[tablename='me_lyhtztbg_bdxx']").find(".listRow").hide();	/*【月度租金分解】不能直接控制隐藏，否则权限控制不了,先隐藏全部，在显示个体*/
	$("div[tablename='me_lyhtztbg_wyffj']").find(".listRow").hide();	/*【物业费分解】不能直接控制隐藏，否则权限控制不了,先隐藏全部，在显示个体*/
});
