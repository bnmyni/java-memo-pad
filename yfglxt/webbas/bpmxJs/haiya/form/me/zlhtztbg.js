...
$(function(){
	...
	initXSJEQvalidateRule(); //样式
	$("input[name='m:me_zlhtztbg:hth']").change(function(){
		var hth = $(this).val();
		if(!hth) return;
		if(hth.length != 9){
...
function validateZujinBiaozhunDates(curRow){ 
	//租赁有效期起，止
	var zuLinStart  =$("[name='m:me_zlhtztbg:zlyxqq']");
...
function loadChangeSffs(){
	var sffs = $("[name$='sffs']:visible",$("[tablename='me_zlhtztbg_mysfxm']"));
...
$().ready(function (){
	...
	$("div[tablename='me_zlhtztbg_ydzjfj']").find(".listRow").hide();	/*【月度租金分解】不能直接控制隐藏，否则权限控制不了,先隐藏全部，在显示个体*/
	$("div[tablename='me_zlhtztbg_wyffj']").find(".listRow").hide();	/*【物业费分解】不能直接控制隐藏，否则权限控制不了,先隐藏全部，在显示个体*/
});
