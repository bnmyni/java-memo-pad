...
function me_zlhtbg_zjbzxxAddRowAfterEvent(row){
	//取出最后一行序号、再次基础上加
	var preRow = $(".listRow:visible",$("[tablename='me_zlhtbg_zjbzxx']")).not(row).last();
...
function validateZujinBiaozhunDates(curRow){
	//租赁有效期起，止
	var zuLinStart  =$("[name='m:me_zlhtbg:yzlyxqq']");
	...
	//原数据处理
	var rows = $(".listRow:visible",$("[tablename='me_zlhtbg_yzjbzxx']"));
	for(var i=0,row;row=rows[i++];){
		if(i>1){
			// 处理比较颜色
			fillColorByCompare(row,$(rows[i-2]),"dj"); // 单价
...
function calZongWuYeFei(obj){
	...
	if(baoDiType =="0"){ //按月
		...
		var days = FormDate.dateVal(startDate, endDate);
...
function loadChangeSffs(){
	var sffs = $("[name$='sffs']:visible",$("[tablename='me_zlhtbg_mysfxm']"));
...
function validateMysfxm (){
	var zuLinStartDate  =$("[name='m:me_zlhtbg:yzlyxqq']").val();
	var zuLinEndDate  =$("[name='m:me_zlhtbg:zlyxqz']").val();
...
function me_zlhtbg_zjbzxxDelRowBeforeEvent(row){
	...
	$("[name='s:me_zlhtbg_ydzjfj:xh'][value=" + xh + "]").closest("tr").remove();/*清除旧的记录*/
}
function me_zlhtbg_wyftkDelRowBeforeEvent(row){
	var xh = $("[name$=':xh']",row).val();
	$("[name='s:me_zlhtbg_wyffj:xh'][value=" + xh + "]").closest("tr").remove();/*清除旧的记录*/
}
/**
 * 修改收费规则的时候删除所有分解
 * wyffj /ydzjfj*/ 
function delAllFenJie(target){
	$("[name='s:me_zlhtbg_"+target+":xh'][value!='']").closest("tr").remove();
}
...
$().ready(function (){
	...
	$("div[tablename='me_zlhtbg_ydzjfj']").find(".listRow").hide();	/*【月度租金分解】不能直接控制隐藏，否则权限控制不了,先隐藏全部，在显示个体*/
	$("div[tablename='me_zlhtbg_wyffj']").find(".listRow").hide();	/*【物业费分解】不能直接控制隐藏，否则权限控制不了,先隐藏全部，在显示个体*/
});
