...
function changeTYShouYin(obj){
	var type = $("[name='m:op_spkysq:tysy']").val();
	if(type==0) $(".tongyishouyin").hide();
	if(type==1) $(".tongyishouyin").show();
}
/* 显示租户信息 */
function showZh(){
	var zhid = $("[name='m:op_spkysq:zhID']").val();
	if(zhid == null || zhid == ""){
...
function calQizuRi(){
	...
	}else  $("[name='m:op_spkysq:qzr']").val(showPuData.Format());
}
...
function decomposeSingle(selectRow,fenJieTable){
	...
	if(fenJieTable == 'op_spkysq_wyffj'){
		...
		var sfgz =  $("[name='m:op_spkysq:wyfsfgzID']").val();
		var type =$("[name$='glfxx']").val();
	// 标准租金
	}else{
		var zj =FormUtil.commaback($("[name$='zj']",selectRow).val()); 
		var sfgz =$("[name='m:op_spkysq:zjsfgzID']").val();
